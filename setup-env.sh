...
API_ENV="popclip-api/.env.dist"
API_LOCAL_ENV="popclip-api/.env"

VD_ENV="popclip-video-downloader/.env"
VD_LOCAL_ENV="popclip-video-downloader/.env.local"

SE_ENV="popclip-sound-extractor/.env"
SE_LOCAL_ENV="popclip-sound-extractor/.env.local"

SG_ENV="popclip-subtitle-generator/.env"
SG_LOCAL_ENV="popclip-subtitle-generator/.env.local"

SM_ENV="popclip-subtitle-merger/.env"
SM_LOCAL_ENV="popclip-subtitle-merger/.env.local"

ST_ENV="popclip-subtitle-transformer/.env"
ST_LOCAL_ENV="popclip-subtitle-transformer/.env.local"

# Check if source files exist
if [ ! -f "$API_ENV" ] || [ ! -f "$VD_ENV" ] || [ ! -f "$SE_ENV" ] || [ ! -f "$SG_ENV" ] || [ ! -f "$SM_ENV" ] || [ ! -f "$ST_ENV" ]; then
    echo "Source environment files not found"
    exit 1
fi

rm -r "$API_LOCAL_ENV" "$VD_LOCAL_ENV" "$SE_LOCAL_ENV" "$SG_LOCAL_ENV" "$SM_LOCAL_ENV" "$ST_LOCAL_ENV" 2> /dev/null

# Create local copies
cp "$API_ENV" "$API_LOCAL_ENV"
cp "$VD_ENV" "$VD_LOCAL_ENV"
...
replace_env_vars "$VD_ENV" "$VD_LOCAL_ENV"
